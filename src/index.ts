...
import {
  ...
  getLastWeb3ApiName,
  storeLastWeb3ApiName, TokenMaxAbsoluteAmount,
  Web3Api,
  Web3ApiEvent,
  Web3ApiProvider,
  Web3Common
  ...
  async activateSubscriptionInWallet(subscriptionGetter: ()=>Promise<Subscription>){
    await this.useApiLock('activateSubscription', async ()=>{
      ...
      if(amountToPay.gt(curAllowance)){
        ...
        const approveAmount = TokenMaxAbsoluteAmount;
        const resultTx = await Web3Common.walletTokenApprove(wallet, token, address, contractAddress, approveAmount);
